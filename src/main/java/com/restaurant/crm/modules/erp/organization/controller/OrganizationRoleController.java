...
public class OrganizationRoleController {
    ...
    @PreAuthorize("@employeeAccessChecker.canManage(null)")
    ...
    @PreAuthorize("@employeeAccessChecker.canManage(null)")
    public ResponseEntity<ApiResponse<List<OrgRoleResponse>>> list(@RequestParam String organizationId) {
        return ResponseEntity.ok(ApiResponse.<List<OrgRoleResponse>>builder()
                .success(ApiConstant.SUCCESS).data(service.listByOrganization(organizationId)).build());
    }
